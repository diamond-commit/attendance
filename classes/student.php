<?php
     ...
     // student class 
       class student {
        ...
        public function __construct( $conn){  
         ...
         public function signIn($name,$lastname, $middlename, $course, $device, $number){
          try {
           // sql query to insert sign in info 
            $sql = "INSERT INTO student(name, lastname, middlename, course, device, number) VALUES (?,?,?,?,?,?)";
            // sql injection 
            ...
            if(!$stmt->execute([$name,$lastname, $middlename, $course, $device, $number])){
             return   ["success" => false, "message"=> "Saving info failed"];
                }
       ...
       return ["success" => false, "message"=> $th->getMessage()];
         ...
         // method to show all people signed in for that day
          public function attendance(){
            try {
              $sql = "SELECT id,lastname, middlename, name, number, course, device, timein , timeout FROM student WHERE DATE(timein) = CURRENT_DATE";
              $stmt = $this->conn->prepare($sql);
            if(!$stmt){
  ...
  return   ["success" => false, "message"=> "content cant be fetched", "result"=>$result];
    ...
    try {
        $sql = "UPDATE student SET timeout = NOW() WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
      ...
      public function search($name){
          try {
    $sql = "SELECT id, name,number,lastname, middlename,  course, device, timein, timeout FROM student
      ...
      if(count($result) >0){
          return   ["success" => true, "message"=> "studnt has been found", "result"=>$result];
       } else{
        return   ["success" => false, "message"=> "student could not be found", "result"=>$result];
      }
    ...
    // function to search with date 
      public function dateSearch($date){
        try {
 $sql = "SELECT id, name, course, device,number, lastname, middlename,  timein, timeout  FROM student where date(timein) = ?";
           $stmt = $this->conn->prepare($sql);
        if (!$stmt) {
            return ["success" => false, "message" => "Preparing query failed"];
        }
